...
export async function main(ns: NS) {
	...
	ns.tprint("Server Name       | Skill | Max Money")
	ns.tprint("------------------+-------+----------");

	for (let i = 0; i < targets.length && i < 10; i++) {
		const server = ns.getServer(targets[i]);

		ns.tprint(`${server.hostname.padEnd(17)} | ${server.requiredHackingSkill.toString().padStart(5)} | ${ns.formatNumber(server.moneyMax).padStart(8)}`);
	}
}
